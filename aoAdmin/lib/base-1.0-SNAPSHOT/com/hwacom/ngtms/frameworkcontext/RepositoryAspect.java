...
/*  64 */             entityClass = (Class)pt.getActualTypeArguments()[0]; break;
/*     */           } 
/*     */         } 
/*     */       } 
...
/*  73 */       logger.warn("getType have exception!.", ex);
/*     */       return;
/*     */     } 
...
/* 151 */     for (String s : tableList) {
/* 152 */       sb.append(s + " ");
/*     */     }
/* 154 */     Map<Class<?>, String> abstractEntitisMap = new HashMap<>();
/* 155 */     logger.info("RPT use history database with {" + sb.toString() + "}");
/* 156 */     for (EntityType<?> e : (Iterable<EntityType<?>>)this.em.getEntityManagerFactory().getMetamodel().getEntities()) {
/* 157 */       if (e.getJavaType() != null && e
/* 158 */         .getJavaType().getAnnotation(Table.class) != null && 
/* 159 */         !((Table)e.getJavaType().getAnnotation(Table.class)).name().isEmpty() && 
/* 160 */         Modifier.isAbstract(e.getJavaType().getModifiers())) {
/* 161 */         String tableName = ((Table)e.getJavaType().getAnnotation(Table.class)).name().toLowerCase();
/* 162 */         abstractEntitisMap.put(e.getJavaType(), tableName);
/*     */       } 
/*     */     } 
/*     */     
/* 166 */     for (EntityType<?> e : (Iterable<EntityType<?>>)this.em.getEntityManagerFactory().getMetamodel().getEntities()) {
/* 167 */       if (e.getJavaType() != null) {
/* 168 */         for (Class<?> clazz : abstractEntitisMap.keySet()) {
/* 169 */           if (clazz.isAssignableFrom(e.getJavaType())) {
/* 170 */             String tableName = abstractEntitisMap.get(clazz);
/* 171 */             if (!tableRepoMap.containsKey(tableName)) {
/* 172 */               tableRepoMap.put(tableName, new LinkedList<>());
/*     */             }
/* 174 */             ((List<String>)tableRepoMap.get(tableName)).add(e.getJavaType().getSimpleName());
/*     */           } 
/*     */         } 
/*     */       }
/*     */     } 
/* 179 */     for (EntityType<?> e : (Iterable<EntityType<?>>)this.em.getEntityManagerFactory().getMetamodel().getEntities()) {
/* 180 */       if (e.getJavaType() != null && e
/* 181 */         .getJavaType().getAnnotation(Table.class) != null && 
/* 182 */         !((Table)e.getJavaType().getAnnotation(Table.class)).name().isEmpty()) {
/* 183 */         String tableName = ((Table)e.getJavaType().getAnnotation(Table.class)).name().toLowerCase();
...
/*     */         }
/* 190 */         ((List<String>)tableRepoMap.get(tableName)).add(e.getJavaType().getSimpleName());
/*     */       } 
/*     */     } 
/* 193 */     for (String table : tableList) {
/* 194 */       if (tableRepoMap.containsKey(table)) {
/* 195 */         for (String repo : tableRepoMap.get(table)) {
