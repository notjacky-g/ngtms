...
public interface ProhibitNodeRepository extends JpaRepository<ProhibitNode, Long> {
  List<ProhibitNode> findByGroupNameAndFmeName(String paramString1, String paramString2);
  
  void deleteByGroupName(String paramString);
  
  @Modifying
  @Query("delete from ProhibitNode p where  p.groupName=(?1) and p.id not in (?2)")
  void deleteByIdNotInGroup(String paramString, Set<Long> paramSet);
}
