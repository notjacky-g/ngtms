...
function createCode() {
	$.ajax({
		type: 'GET',
		url: location.protocol + "//" + window.location.hostname + ":" + window.location.port + "/first/checkNginxActivated",
		dataType: 'json',
		async: true,
	    success: function (response) {
	       if (response == true) {
	    	   genValidateCode(location.protocol + "//" + window.location.hostname + ":" + window.location.port);
	       } else {
	           $.ajax({
	    	     type: 'POST',
...
function login() {
	$.ajax({
	    ...
	    success: function (response) {
	       if (response == true) {
	    	   loginValidate(location.protocol + "//" + window.location.hostname + ":" + window.location.port);
	       } else {
	           $.ajax({
	    	     type: 'POST',
	    	     url: location.protocol + "//" + window.location.hostname + ":" + window.location.port + "/first/retrieveSpecificIpAndPort",
